...
| AMD-001 10.6, OWASP Top 10 testing before deployment | **Met for pre-deployment.** 82 checks across the ten categories over the HTTP surface in both modes, none failed; two observations (single role, no alerting) and the TLS check deferred to after first deploy. Record: `docs/OWASP-TOP-10-TEST.md`. The annual external test is AMD-001 11.5, September 2026. | Met |
| AMD-001 10.4, accreditation review before production deployment | **Not yet.** Required before the first deploy. The review and the decision are the UK Information Security Officer's (Ash Higgins), recorded in Library 04. | Open |
