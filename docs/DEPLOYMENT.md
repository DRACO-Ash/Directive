...
● ~~The platform pipeline simulation has not been run against a package artefact.~~ **Closed.** `scripts/build-package.sh` and `scripts/simulate-pipeline.sh` exist, are documented under The upload package above, and the simulation returns `SIMULATION: PASS` against the built artefact. `tests/test_package_build.py` exercises the build's refusals in a throwaway clone, which nothing did for three consecutive security gates, and the count is stated rather than implied: nineteen tests covering the archive fidelity check, the credential sweep across encodings and filenames, the exemption scope and its pinned count, the name sweep, the symlink and nested-Dockerfile refusals, the version guard, the required-file list, and both of the pipeline simulation's guards. The sweep's `OSError` branch is NOT exercised: `git archive` creates the staged files itself, so an unreadable one cannot be arranged from outside the script. Recorded rather than left to be discovered.
