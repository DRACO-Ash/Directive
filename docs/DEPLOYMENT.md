# directive deployment parameters
...
| Setting | Value |
|---|---|
| App slug | `directive` (single hyphen; a double hyphen fails the pipeline with zero stages run) |
...
| `DIRECTIVE_ENV` | Operator-set, not secret | **Set this to `production`, explicitly.** It governs three fail-closed controls: the `Secure` flag on the session cookie, whether Entra ID is enforced at boot, and whether `SESSION_KEY` is required. `development` selects the local mode; anything else, an unset variable included, is production. It was previously absent from this table while defaulting to `development`, so a deploy following this table exactly ran with all three guards off. The default is now the safe direction, and the row exists so nobody has to rely on it. |
| `TENANT_ID` | Operator-set, not secret | The Bluestaq Ltd Entra ID tenant identifier. |
| `CLIENT_ID` | Operator-set, not secret | The directive application registration identifier. |
| `REDIRECT_URI` | Operator-set, not secret | `https://directive.apps.bluestaq.com/auth/callback` |
...
5. **Re-anchor.** First obtain the entries-ever total from OFF this volume: the figure
   ...
   `directive.audit.re_anchor(DATA_DIR, outgoing_key=..., incoming_key=..., expected_total=N)`
...
● Rate limiting is not implemented. When it lands, `/readyz` and `/api/diagnostics` both belong behind the broad limiter. The storage probe's per-request amplification is closed: the probe is single-flight, so concurrent callers join one probe rather than each starting a thread. That is the probe alone, not the process: the bullet below names one that is still open.
● **`/auth/callback` is not single-flight, and that is the remaining per-request amplifier.** Each request on that route drives one outbound token POST to Entra ID, bounded by `TOKEN_TIMEOUT_SECONDS` at 10 seconds, and it is unauthenticated by necessity. The storage probe was made single-flight because concurrent callers each started a thread; this one cannot be, because each exchange carries a different authorisation code and a shared result would be wrong. Measured by the security gate against a stalled token endpoint: eight concurrent exchanges held all eight threads of the single gthread worker and pushed `/readyz` to **6.17 seconds**, against a `HEALTHCHECK` timeout of 4 seconds, so a sustained burst can make a healthy container look unhealthy and be restarted. Not closed here: the fix is the rate limiter above, applied to `/auth/callback` ahead of the exchange, and the refusal collapsing in `views/refusals.py` already bounds what such a burst can write to the log. `TBC, re-verify` after the first deploy, because the platform ingress may absorb the burst before it reaches the pod and the real Entra ID latency is not the stalled one measured here.
...
● ~~The platform pipeline simulation has not been run against a package artefact.~~ **Closed.** `scripts/build-package.sh` and `scripts/simulate-pipeline.sh` exist, are documented under The upload package above, and the simulation returns `SIMULATION: PASS` against the built artefact. `tests/test_package_build.py` exercises the build's refusals in a throwaway clone, which nothing did for three consecutive security gates. **The coverage claim here has been wrong three times, so it names no figure at all.** It claimed one unexercised branch when a deletion matrix found twenty-one; it then claimed controls as held that a mutation showed were not, including the pointer race and the exemption match count, because the tests naming them were satisfied by a different control; and it pointed at a deletion matrix that was not recorded anywhere. **The authoritative record is the Deletion matrix section of `docs/GATE-RECORDS.md`, which carries a row for each run in which the matrix was actually measured, not for every gate run.** Read that, not this. The sentence here claimed a row per run and it was wrong, which is the fourth consecutive release in which this one paragraph has promised more evidence than the record behind it holds. Two things are worth stating here because they do not change between runs: the sweep's `OSError` branch cannot be exercised from outside the script, since `git archive` creates every staged file itself, and a hostile committer who edits the exemption allowance itself is outside what any of this defends against, by the same shared-trust argument that lets a committer change the application directly.
...
this application is not classified. `docs/policy/README.md` names them and says how to obtain
...
| AUD-001, event field set | **Met for four categories, deviated for three.** One fixed shape rather than one per category, because a digest over a varying field set cannot be verified without knowing the variant. Authentication, Task management, Register operations and Audit export map onto `FIELD_ORDER` directly. Incident management and Administration are covered by the old-and-new-value deviation below. **Form submissions asks for "key field values" and no field can carry a value**, so that clause is unimplementable under the same decision and is recorded as a deviation in its own right, not covered by the row below. | `FIELD_ORDER`; AUD-001 clause, see `docs/policy/README.md` |
| AUD-001, old and new value of a changed field | **Deviated, deliberately, and the deviation is weaker than first claimed.** Field NAMES only in `fields_changed`, capped at 256 bytes in total and at 48 bytes per NAME. The per-name cap is the structural form of what the total was approximating, and it strengthens this deviation rather than weakening it: a total cap cannot tell a field name from a sentence written like one, and the 79-byte snake-case sentence it was tightened to 128 for passed that 128 on its own. The per-name rule refuses it at the first occurrence. The total moved from 128 to 256 because a transfer risk assessment naming all thirteen of its declared fields AND a starting state joins to 132 bytes and could not be created at all. The thirteen names alone come to 126, which fits; it is the state that carries it over, and this row said 132 of the thirteen until a gate did the arithmetic. This row read 128 after both figures changed, which is the kind of stale deviation an assessor is entitled to catch; an enumerated workflow state in `old_state` and `new_state` under a character rule that rejects the common SHAPES of record content (a space, lower case, an `@`, over 32 characters) but does not make it impossible: a single upper-case token such as `HIGGINS` or `SW1A1AA` satisfies it. A closed state vocabulary IS now defined, per register, in `records.py`, one tuple for each of the five registers (`TASK_STATES`, `INCIDENT_STATES`, `RISK_STATES`, `AGREEMENT_STATES` and `TRANSFER_STATES`), and `check_state` enforces it on every REGISTER route. This row named only the first three after the transfer registers shipped, which is how a stale vocabulary claim looks. It is bounded rather than structural, and be exact about the bound: the authentication and refusal path writes a collapsed-count marker (`REPEATED_<n>`) into `new_state` directly, under the character rule only, which is the one declared exception and is an event count rather than a register state. The audit boundary itself still holds only the character rule, so a caller that bypassed `check_state` would not be stopped there, and caller discipline remains load-bearing for that path. This row previously said the set was not knowable until the records module, which shipped, and then that `check_state` covered every route, which the refusal path falsifies. The vocabulary stays `TBC, re-verify` with the ISM. Ash's decision, recorded for sign-off; the vocabulary is `TBC, re-verify`. | `src/directive/audit/validation.py` |
...
| AMD-001 10.6, security headers on all responses | **Met.** All four named headers, plus three more, on every response including probes, redirects and error pages, applied OVER anything a route set. The previous implementation was first-writer-wins, so a route could serve a wider policy and keep it; a narrower per-route policy now goes through an explicit `tighten` call and there is no door for a wider one. | `src/directive/security_headers.py`, `tests/test_security_headers.py` |
...
● **Form submissions, "key field values"** (AUD-001, see `docs/policy/README.md`). No field in
