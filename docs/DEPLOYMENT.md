...
| `/api/diagnostics` | Diagnostics | Booleans, counts, and length BANDS only. Never a secret value, and never an exact length. It does echo `buildId`, `dataDir` and `port`, which are operational rather than secret. |
...
| `AUDIT_HMAC_KEY` | Operator-set, SECRET | The audit signing key. Must be real key material: hexadecimal or base64 decoding to at least 32 bytes. Generate with `openssl rand -hex 32`. A passphrase is refused, because every stored row is a message and its tag and the list is readable by more people than hold the key, so a low-entropy value falls to an offline attack and returns full re-stamping power over the audit log. Without a key no audit entry can be written: the chain fails closed rather than writing an unsigned entry. The value is used verbatim and never trimmed or unquoted, so do not wrap it in quotes. |
| `AUDIT_KEY_ID` | Operator-set, not secret | `k1` for the first key. Recorded on every entry it signs. |
| `AUDIT_RETIRED_KEYS` | Operator-set, SECRET | Empty until the first rotation. Then `id:key` pairs separated by semicolons, each key held to the same bar as the current one, so entries signed before the rotation stay verifiable. A retired key remains a valid signer for history but cannot sign the END of the log, so a leaked retired key cannot be used to re-sign the whole thing. |

## Add-ons

| Add-on | Needed | Why |
|---|---|---|
| FILE_STORAGE | Yes | The server-side session store and the audit anchor must survive a restart. The anchor is the trusted record of where the audit log should end, and it is what detects a wholesale rewrite or a truncation of the log; it is written atomically and durably to this volume, authenticated under the signing key so it cannot be forged by something with volume access but no key. Injects `STORAGE_MOUNT_PATH=/data`. |
...
● The audit anchor's residual risk, stated rather than implied. An actor with write access to the persistent volume who KEEPS a genuine older anchor can restore it alongside a matching truncation of the log, and the result is indistinguishable from that earlier moment in time. A rollback within a running process is refused, but that state does not survive a restart, and closing the offline case needs a trusted record outside this container. The compensating controls are SharePoint list versioning and retention on the list itself, and the operator exporting the anchor into the evidence library on a schedule. `TBC, re-verify` both with the ISM.
