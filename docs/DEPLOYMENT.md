...
● ~~The platform pipeline simulation has not been run against a package artefact.~~ **Closed.** `scripts/build-package.sh` and `scripts/simulate-pipeline.sh` exist, are documented under The upload package above, and the simulation returns `SIMULATION: PASS` against the built artefact. `tests/test_package_build.py` exercises the build's refusals in a throwaway clone, which nothing did for three consecutive security gates. **The coverage claim here has now been wrong twice, so it is stated as a pointer rather than a figure.** It first named one unexercised branch when a deletion matrix found twenty-one controls surviving deletion; the replacement then claimed the exemption allowance was held by match count when that was itself defeated, and claimed four simulation guards where the script has more. Thirty-three tests now hold the archive fidelity check, the credential sweep across encodings and paths, the exemption allowance by path, match count and line digest, the name sweep, the symlink, nested Dockerfile, version and required-file refusals, the dirty stamp, the pointer invalidation, the pointer race, and the pipeline simulation's red-suite, no-package, digest, skip-bit, dirty-tree, dirty-stamp and wrong-commit guards. **The authoritative count of what is NOT held is the security gate's deletion matrix, recorded per run in `docs/GATE-RECORDS.md`, not a number written here.** The sweep's `OSError` branch is known to be unexercisable from outside the script, because `git archive` creates every staged file itself.
