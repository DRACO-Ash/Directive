...
| A false figure can be put in the price passage as a vague count the ban does not name, such as `a few` or `most of them` | Twenty-sixth pass, `936d6dd` | The near end of that class is banned, which is where every walk-around so far has come from. English has more ways to say a number than a list can hold, so the list is declared as the words it names rather than as a closure over counts written as words. The digits are held separately and exactly. |
| A false figure can be put in a NEW paragraph of the sweep's comment block, or a NEW bullet below the record's, outside the passage bound | Thirty-first run and twenty-sixth pass, `936d6dd` | Needs a deliberate structural edit, so the adversary is a committer to this repository, who can edit the test instead. A whole-file negative check is not the alternative: both carriers name every one of these modules elsewhere for unrelated reasons, so it would be red always. |
| `both` and `one` are excluded from the word ban | Twenty-sixth pass, `936d6dd` | Each is used in the passage for something other than the price - `one` as a pronoun, `both` for the keyword group's two words - so banning it buys a worse sentence rather than a held figure. |
...
● A bare `key` or `keys` in the QUOTED rule's keyword group, so `key = "<value>"` in source
  ...
  above, asked for one. Measured separately from the unquoted rule's figure above, because
  it is a different
  ...
  bans one anywhere in this bullet, in either case, with the ordinals that name a gate run
  removed first. The experiment is to add `key|keys` to that rule's group alone and scan
  every tracked file.
