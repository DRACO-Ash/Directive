...
| 2026-09-12 | `engineering-reviewer`, twenty-fourth pass | `ac3d4ea` | **FAIL** | Four MAJORs, all in prose, three of them in sentences that commit wrote to fix prose defects. It found the same 400-character window the thirtieth security run found, independently and with the same defeat, and added the measurement that the window fitted neither carrier: 13 characters short of the sweep's comment block and 6 characters into the next bullet of this record. It caught this table crediting the route-map walk to `1537b62` when `git log -S` gives `9b3bba3` alone, contradicting the row three lines above it. It caught `Those three module names`, a count written as a word, free to become `seven` in both carriers with the whole suite green, in the very test written to close that class. And it measured the walk's docstring claim that `HEAD` on every gated route answers 401 against `/console`, which redirects. The word ban that closes the third is new: digits are held by the shape sweep, words are now banned outright in the price passage, hyphenated ordinals excepted. |
...
● A bare `key` or `keys` in the QUOTED rule's keyword group, so `key = "<value>"` in source
  ...
  the scan does not give. No count is restated anywhere in this bullet, because a count
  written as a word is invisible to any scanner and was free for exactly that reason. The
