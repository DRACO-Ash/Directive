...
● **An authenticated actor** doing something they should not, whether careless or hostile. This category is nearly empty by design and saying so is more honest than listing it as though it had content: there is ONE role, so every assigned user can read and write every register, and the boundary is what the application refuses everyone plus the Entra assignment list. The audit log is what makes an action attributable afterwards. Recorded as an observation, not a control, in `docs/OWASP-TOP-10-TEST.md` under A01.
...
● **Anyone reading the repository or the package.** No secret in any shippable file, in source or in history. The credential sweep over the staged package is part of this, not of the build-host model below. **Be exact about whose mistake the sweep catches: an HONEST committer's, not a hostile one's.** The pre-write hook only sees this assistant's edits, so a human `git add`, a heredoc or a paste never passes through it, and the sweep is the net under that. A hostile committer can edit the sweep, which is why they are excluded below. That distinction is not pedantry, it decides how complete the pattern set has to be: the accident this repository will actually have is a dotenv-shaped `CLIENT_SECRET=...` in the one file the package is required to ship, and the rule set was tuned for a source-code-shaped `secret = "..."` until a gate demonstrated the gap.
...
● Every response carries the AMD-001 section 10.6 security headers: Content-Security-Policy, Strict-Transport-Security, X-Content-Type-Options, and X-Frame-Options. Applied over anything a route set, so tighten-only is mechanical rather than a convention: a narrower per-route policy goes through `security_headers.tighten`, and there is no door for a wider one.
